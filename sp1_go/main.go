...
//export MakeWitness
func MakeWitness(sp1WitnessJsonPath *C.char, witnessBinPath *C.char) {
	start := time.Now()
	// Read the file.
	data, err := os.ReadFile(C.GoString(sp1WitnessJsonPath))
	if err != nil {
		panic(err)
	}
	...
	file, err := os.Create(C.GoString(witnessBinPath))
	if err != nil {
		panic(err)
	}
	defer file.Close()
